        ...
        <div className="mt-4 flex gap-2 overflow-x-auto no-scrollbar -mx-5 px-5">
          {muscleFilters.map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`press-scale shrink-0 rounded-full px-4 py-2 text-xs font-medium transition-all ${
                filter === f
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 pb-4">
          {filtered.map((ex) => (
            <button
              ...
              className="press-scale overflow-hidden rounded-2xl bg-card text-left"
